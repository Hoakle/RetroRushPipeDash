...
namespace RetroRush.Game.Level
{
    public class PipeFace : GraphicalObjectRepresentation<PipeFaceData>
    {
        ...
        private void SetPositionAndRotation()
        {
            ...
            transform.position = new Vector3(Data.Position.x, Data.Position.y, parentPos.z + (_MeshTransform.localScale.z * Data.Depth) + _MeshTransform.localScale.z / 2 ); 
        ...
        public void SetObstacle()
        {
            ...
            {
                lazer.transform.localScale = new Vector3(0.2f, (float) Math.Sqrt(Math.Pow(transform.position.y - Data.LinkedFace.Position.y, 2) + Math.Pow(transform.position.x - Data.LinkedFace.Position.x, 2)) / 2f, 0.2f);
                
                lazer.transform.SetParent(transform, false);
                
                lazer.transform.position = new Vector3((transform.position.x + Data.LinkedFace.Position.x) / 2, (transform.position.y + Data.LinkedFace.Position.y) / 2, transform.position.z);
        ...
        {
            transform.RotateAround(transform.parent.position, Vector3.back, Data.RotateAround - transform.parent.eulerAngles.z);
